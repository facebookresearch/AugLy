...
def loop_audio(
    ...
    """
    Takes in an audio file/numpy array and returns it looped a given n number of times
    @param audio: the path to the audio or a variable of type np.ndarray that
        will be augmented
    ...
    # or possibly:
    # aug_audio = np.repeat(audio, 7, axis=0)
    # aug_audio = aug_audio.flatten()

    audutils.get_metadata(
        metadata=metadata,
        function_name="loop_audio",
        audio=audio,
        sample_rate=sample_rate,
        dst_audio=aug_audio,
        dst_sample_rate=sample_rate,
